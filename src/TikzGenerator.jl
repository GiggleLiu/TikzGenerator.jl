...
function Node(x, y;
    ...
    props = build_props(;
        fill = fill,
        draw = draw,
        inner_sep = "$(inner_sep)cm",
        minimum_size = "$(minimum_size)cm",
        line_width = "$(line_width)cm",
        kwargs...)
...
function Mesh(xmin, xmax, ymin, ymax; step=1.0, draw="gray", line_width=0.03, kwargs...)
    Mesh(xmin, xmax, ymin, ymax, build_props(; step="$(step)cm", draw=draw, line_width=line_width, kwargs...))
end
